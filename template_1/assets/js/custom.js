...
(function( $ ){
	...
	/* ----------------------------------------------------------- */
       /*  var objConnection = new ActiveXObject("adodb.connection");
         var strConn = "driver={sql server};server=Localhost;database=db_b140622cs;uid=b140622cs;password=b140622cs";
         objConnection.Open(strConn);
         var rs = new ActiveXObject("ADODB.Recordset");
         var strQuery = "SELECT * FROM Info WHERE year='2050'";
         rs.Open(strQuery, objConnection);
         rs.MoveFirst();
        while (!rs.EOF) {
             var date = rs.fields('start_date');
          }
  */

	var date='0000/10/06';
	$('#mu-event-counter').countdown(date).on('update.countdown', function(event) {
